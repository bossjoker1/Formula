...
我现在在做fuzzing，目的是针对合约中可能存在的价格操纵漏洞或者其它逻辑缺陷漏洞导致的被套利。这类漏洞
与传统的如整数溢出、重入、访问控制等漏洞有很大区别，它通常与状态变量很相关，并且一般需要多比交易来触发。
受害合约会通常与Pancake(Uniswap) Router、Pair等合约还有受害Token(ERC20)，以及其它稳定币或WBNB/WETH进行交互。因此，我认为Pair合约各token的reserve，totalsupply, allowance[], balanceOf[]这些状态变量的写操作是我们需要重点关注的。并且与是否作为fuzzing的语料库以及优先级的高低有关。

第一步我首先提供给你Router, Pair合约，你需要分析它的所有external/public函数。主要分析它们针对我上面所说的reserve，totalsupply, allowance[], balanceOf[]的写操作是什么。由于，这类操作经常涉及两种token，你可以统一为token_A, token_B进行命名。

具体的，你需要给定这些函数，它们对于上述状态变量的影响是什么，减少、增加还是其它的操作，这部分你需要进行程序分析的抽象解释，定义抽象域，抽象表示，抽象操作(我需要投软工类的会议，因此这部分需要严谨且学术化)。

你说先知晓并弄清楚我的需求，然后等我进一步的输入。
